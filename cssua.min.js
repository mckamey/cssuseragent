...
"",c;for(c in b)if(c&&b.hasOwnProperty(c)){if(a)a+="&";a+=encodeURIComponent(c)+"="+encodeURIComponent(b[c])}return a}};g.userAgent=g.ua=g.parse(l);var k=g.format(g.ua);if(i.className){i.className=i.className.replace(/no-js/gi,'');i.className+=k;}else i.className=k.substr(1);i.className+=' js';return g}(document.documentElement,navigator.userAgent);